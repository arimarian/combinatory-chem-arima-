...
(defn gillespie-sample-reaction
  ...
  (let [reaction-type->propensity (gillespie-total-propensity-by-type pool)
        bias-factors {:cleave 1.2  ; :cleave 反応の確率を上げる
                      :condense 0.8 ; :condense 反応の確率を下げる
                      :reduce 1.0}  ; :reduce 反応の確率は変更しない
        biased-reaction-type->propensity (->> reaction-type->propensity
                                            (map (fn [[type prop]]
                                                   [type (* prop (get bias-factors type 1.0))]))
                                            (into {}))
        total-reaction-probs (normalize-map biased-reaction-type->propensity)
